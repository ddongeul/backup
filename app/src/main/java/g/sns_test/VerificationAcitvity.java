...
public class VerificationAcitvity extends AppCompatActivity {
    ...
    private TextView textview_timeCount, verification_num;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        verification_num = findViewById(R.id.verification_num);

        //인증 제한 시간
        int totalTime = 1000*60*3;
        //시간이 줄어드는 인터벌(1초로 설정)
        int interval = 1000;
        ...
        verification_num.setText(verificationNum);

        //시간을 카운트다운하기 위해서 선언한 객체
        CountDownTimer countDownTimer = new CountDownTimer(totalTime,interval) {

            //생성자의 두번째 param에 지정한 숫자만큼 시간이 지날때마다 호출되는 메소드다. 즉 1초에 한번씩 호출된다고 보면 됨.
